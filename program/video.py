            ...
            if search == 0:
                ...
                if veez == 0:
                    ...
                    if chat_id in QUEUE:
                        ...
                        try:
                            await loser.edit("🔄 **Sesliye Katılıyorum...**")
                            await call_py.join_group_call(
                                chat_id,
                                AudioVideoPiped(
                                    ytlink,
...
async def vstream(c: Client, m: Message):
    ...
    if not a.can_manage_voice_chats:
        await m.reply_text(
            "missing required permission:" + "\n\n»❌ __Sesli Sohbet Yoneticilik yok__"
        )
        return
    if not a.can_delete_messages:
        await m.reply_text(
            "missing required permission:" + "\n\n» ❌ __Mesaj Silme Yetkis yok__"
        )
        return
    if not a.can_invite_users:
        await m.reply_text("missing required permission:" + "\n\n» ❌ __Add users__")
    ...
    try:
        ...
        if m.chat.username:
            try:
                ...
                await m.reply_text(f"❌ **Userbot Sesliye Katılamadı**\n\n**reason**: `{e}`")
                return
        else:
            try:
                user_id = (await user.get_me()).id
    ...
    if len(m.command) < 2:
        ...
        if len(m.command) == 2:
            ...
            Q = 720
            loser = await c.send_message(chat_id, "🔄 **Başlatılıyor...**")
        ...
        if veez == 0:
            await loser.edit(f"❌ Link Hatalı\n\n» `{livelink}`")
        else:
            if chat_id in QUEUE:
                ...
                await m.reply_photo(
                    photo=f"{IMG_1}",
                    caption=f"💡 **Sıradaki Şarkıya Geçiliyor »** `{pos}`\n\n💭 **Gurup:** `{chat_id}`\n🎧 **Talep Eden:** {requester}",
                    reply_markup=keyboard,
                )
            else:
                if Q == 720:
                ...
                try:
                    ...
                    await m.reply_photo(
                        photo=f"{IMG_2}",
                        caption=f"💡 **[Video live]({link}) Başlatılıyor .**\n\n💭 **Gurup:** `{chat_id}`\n💡 **Durum:** `Oynatılıyor`\n🎧 **Talep eden:** {requester}",
                        reply_markup=keyboard,
                    )
                except Exception as ep:
                    await loser.delete()
